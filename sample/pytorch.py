...
class Model(nn.Module):
    def __init__(self):
        ...
        self.conv21 = nn.Conv2d(
            2 * nbf, 2 * nbf, kernel_size=(3, 3), padding=1, bias=True
        )
        ...
        self.conv23 = nn.Conv2d(
            2 * nbf, 4 * nbf, kernel_size=(3, 3), padding=1, bias=True
        )
...
model = Model()
input0 = (
    np.linspace(-1.0, 1, np.prod(s)).reshape((1, h, w, 3)).astype(np.float32)
